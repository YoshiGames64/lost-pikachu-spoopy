...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		if (!initialized)
		{
			...
			// music.loadStream(Paths.music('menu inst og'));
			// FlxG.sound.list.add(music);
			// music.play();

			if(FlxG.sound.music == null) {
				FlxG.sound.playMusic(Paths.music('menu inst og'), 0);
			}
		}
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 2:
					...
					#else
					createCoolText(['yoshimod420', 'maddo']);
					#end
				// credTextShit.visible = true;
				case 4:
					#if PSYCH_WATERMARKS
					addMoreText('YoshiMod420', 15);
				...
				// credTextShit.text += '\nNight';
				case 16:
					addMoreText('Unown Update'); // credTextShit.text += '\nFunkin';
